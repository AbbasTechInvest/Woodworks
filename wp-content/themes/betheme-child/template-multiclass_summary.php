 ...
 * Template Name: Multiclass_history
 */
                ...
                if( $rows ) {
                    foreach( $rows as $row ) {
                        ...
                        if($participantID == $current_user_id){
                            ...
                            $booking_date = get_field('payments_date', $row['multiclass_participant_payment']);
                            break; 
                        }
                    }
                }
                if($user_participated) {
                    $booking_status = get_field('multiclass_booking_enabled');
                    $x = [];
                    $x['ID'] = get_the_ID();
                    $x['permalink'] = get_permalink();
                    $x['title'] = get_the_title();
                    $schedule = get_field('multiclass_recurring_dates');
                    $x['date'] = date('d-M-Y', strtotime($schedule[0]['multiclass_session_date']));
                    $x['fee'] = get_field('multiclass_fee');
            ...
            <div class="column mcb-column mcb-item-30126a0aa one laptop-one tablet-one mobile-one column_column"
                ...
                <div class="mcb-column-inner mfn-module-wrapper mcb-column-inner-30126a0aa mcb-item-column-inner">
                    <div class="column_attr mfn-inline-editor clearfix align_center" style="">
                        <h2><?php _e('Recurring-Class Purchase History'); ?></h2>
                    </div>
                </div>
            </div>
                                ...
                                <?php if(is_user_logged_in()) : ?>
                                    <?php if($i + $j > 0): ?>
                                        ...
                                        <?php if($completed_classes): ?>
                                            <table>
                                                <thead>
                                                    <tr style="text-align:center;"><th colspan="5" style="background:#f3d8bb;">Completed Class Details</th></tr>
                                                    <tr>
                                                        <td><b>Class Name</b></td>
                                                        <td><b>Date</b></td>
                                                        <td><b>Fee</b></td>
                                    ...
                                    <?php else : ?>
                                        ...
                                        <table>
                                            <thead><tr style="text-align:center;"><th colspan="5" style="background:#f3d8bb;">Class Details</th></tr></thead>
