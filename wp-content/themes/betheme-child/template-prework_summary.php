<?php
/**
 * The template is for prework purchase history.
 * Template Name: Prework_history
 */
                ...
                if( $rows ) {
                    foreach( $rows as $row ) :
                        ...
                        if($participantID == $current_user_id){
                            ...
                            $booking_date = get_field('payments_date', $row['prework_participant_payment']);
                            break; 
                        }
                    endforeach;
                }
                if($user_participated) {
                    $booking_status = get_field('prework_booking_enabled');
                    $x = [];
                    $x['ID'] = get_the_ID();
                    $x['permalink'] = get_permalink();
                    $x['title'] = get_the_title();
                    $x['date'] = date('d-M-Y', strtotime(get_field('prework_date')));
                    $x['fee'] = get_field('prework_fee');
            ...
            <div class="column mcb-column mcb-item-30126a0aa one laptop-one tablet-one mobile-one column_column"
                ...
                <div class="mcb-column-inner mfn-module-wrapper mcb-column-inner-30126a0aa mcb-item-column-inner">
                    <div class="column_attr mfn-inline-editor clearfix align_center" style="">
                        <h2><?php _e('Prework Purchase History'); ?></h2>
                    </div>
                </div>
            </div>
                                    ...
                                    <?php if(is_user_logged_in()) : ?>
                                        <?php if($i + $j > 0): ?>
                                            <?php if($upcoming_classes): ?>
                                                <table>
                                                    <thead>
                                                        <tr style="text-align:center;"><th colspan="5" style="background:#419bd5;">Upcoming prework session details</th></tr>
                                                        <tr>
                                            ...
                                            <?php if($completed_classes): ?>
                                                <table>
                                                    <thead>
                                                        <tr style="text-align:center;"><th colspan="5" style="background:#f3d8bb;">Completed prework session details</th></tr>
                                                        <tr>
                                                            <td><b>Class Name</b></td>
                                                            <td><b>Date</b></td>
                                                            <td><b>Fee</b></td>
                                        ...
                                        <?php else : ?>
                                            ...
                                            <table>
                                                <thead><tr style="text-align:center;"><th colspan="5" style="background:#f3d8bb;">Session Details</th></tr></thead>
