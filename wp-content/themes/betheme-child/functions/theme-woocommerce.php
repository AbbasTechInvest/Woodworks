	...
	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		<div class="<?php echo implode(' ', $classes); ?>" data-row-key="<?php echo $cart_item_key; ?>" data-product-id="<?php echo $product_id; ?>">
			...
			<div class="mfn-chp-col mfn-chp-info">
				<h6><a style="color: #419bd5; font-size: 1.5em; font-weight: 700;" href="<?php echo $_product->get_permalink(); ?>"><?php echo $_product->get_name(); ?></a></h6>
				<?php do_action( 'woocommerce_after_cart_item_name', $cart_item, $cart_item_key );
				echo wc_get_formatted_cart_item_data( $cart_item ); // PHPCS: XSS ok.
				?>
				<p class="price"><?php esc_html_e( 'Price', 'woocommerce' ); ?>: <b><?php echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); ?></b></p>
				<p class="price"><?php esc_html_e( 'Total', 'woocommerce' ); ?>: <b><?php echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key ); ?></b></p>
			</div>
...
function mfn_tell_free_delivery() {
  ...
  if($total < $free):
  	...
  	if( !empty(get_option('woocommerce_currency_pos')) && get_option('woocommerce_currency_pos') == 'left' ) $diff = get_woocommerce_currency_symbol().round(($free-$total), 2);
