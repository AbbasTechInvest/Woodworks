<?php get_header(); ?>
                ...
                <table>
                    ...
                    <tbody>
                        <?php 
                            ...
                            $booked_seats = 0;
                            if( have_rows('class_participant_details') ){
                                while( have_rows('class_participant_details') ) { 
                                    the_row();
                                    $seat = get_sub_field('class_participant_num_seats');
                                    if(!$seat){ $seat = 1; }
                                    $booked_seats += $seat;
                                }
                            }
                        ...
                        <tr><td>Location</td><td><?php echo get_field('location_name', get_field('class_location')); ?></td></tr>
                        <?php /*<tr><td>Instructor</td><td><?php $instructor = get_field('instructor_name', get_field('class_instructor')); if($instructor) { echo $instructor; } else { echo "Not Assigned"; } ?></td></tr> */ ?>
                        <tr><td>Amount</td><td><?php the_field('class_fee'); ?> KD</td></tr>
                        <?php 
                            ...
                            <?php
                                ...
                                if( $rows ) :
                                    ...
                                    if(!$user_participated && "Open" == $is_open): ?>
                                        ...
                                        <button onclick="book(this)" data-amount="<?php the_field('class_fee'); ?>" id="class_<?php echo get_the_ID(); ?>">Enroll Now</button>
                ...
                <?php 
                    ...
                    <div id="participants">
                        <table>
                            ...
                            <tbody>
                            <?php if( have_rows('class_participant_details') ) : $i=1;
                                while( have_rows('class_participant_details') ) : the_row(); ?>
                                    <tr>
                                        <td><?php echo $i; $participant_user = get_sub_field('class_participant_id'); ?></td>
                                        <td><?php $u = get_sub_field('class_participant_name'); echo "<a href='".get_site_url()."/wp-admin/user-edit.php?user_id=".$participant_user['ID']."'>$u</a>"; ?></td>
                                        <td><?php echo get_sub_field('class_participant_whatsapp'); ?></td>
                                        <td><?php echo get_sub_field('class_participant_email'); ?></td>
                                        <td><?php $pay_id = get_sub_field('class_participant_payment'); echo "<a href='".get_site_url()."/wp-admin/post.php?post=$pay_id&action=edit'>".get_the_title($pay_id)."</a>"; ?></td>
                                        <td><?php echo get_sub_field('class_participant_num_seats'); ?></td>
                                    </tr>
                                <?php $i++; endwhile; ?>
                            <?php else: ?>
    ...
    <script>
        function set_price(e){
            ...
            const id = "#class_" + e.id
            if(seats > booking_limit || seats < 1 || isNaN(seats)){
                e.value = 1;
                jQuery(id).attr('data-amount', <?php the_field('class_fee'); ?>);
                return;
            }
            jQuery(id).attr('data-amount', seats * <?php the_field('class_fee'); ?>);
        }
